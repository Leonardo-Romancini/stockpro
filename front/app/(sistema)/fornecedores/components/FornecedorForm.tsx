...
export default function FornecedorForm({ fornecedorExistente }: FornecedorFormProps) {
    ...
    const handleChange = (campo: 'rzsocial' | 'nomef' | 'cnpj' | 'email', valor: string) => {
        setFornecedor(prev => new Fornecedor(
            prev.id,
    ...
    const handleBuscarCnpj = async () => {
        ...
        setLoadingCnpj(true);
        try {
            const dados = await buscarDadosPorCnpj(fornecedor.cnpj);
            if (dados) {
                setFornecedor(prev => new Fornecedor(
                    prev.id,
                    dados.rzsocial || prev.rzsocial,
                    dados.nomef || prev.nomef,
                    prev.cnpj,
                    dados.email || prev.email,
                    prev.status
                ));
            } else {
                    ...
                    <form action={handleSalvar} className="p-8 md:p-10 space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Razão Social</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Nome Fantasia</label>
                                <input
                                    type="text"
                                    required
                                    className="w-full px-5 py-4 bg-zinc-50 border-2 border-zinc-100 rounded-2xl text-zinc-700 font-bold outline-none focus:border-blue-500 focus:bg-white transition-all placeholder:text-zinc-300"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">CNPJ</label>
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-4 bg-zinc-800 hover:bg-zinc-700 text-white font-bold rounded-2xl transition-all active:scale-95 whitespace-nowrap"
                                    >
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Email de Contato</label>
                                <input
                                    type="email"
                                    required
                                    className="w-full px-5 py-4 bg-zinc-50 border-2 border-zinc-100 rounded-2xl text-zinc-700 font-bold outline-none focus:border-blue-500 focus:bg-white transition-all placeholder:text-zinc-300"
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-end gap-4 pt-6 border-t border-zinc-100">
                            <Link href="/fornecedores" className="w-full sm:w-auto px-8 py-4 text-xs font-black uppercase tracking-widest text-zinc-400 hover:text-zinc-600 transition-colors text-center">
                                Cancelar
                            </Link>
                            <button type="submit" className="w-full sm:w-auto px-10 py-4 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase italic tracking-tighter text-sm rounded-xl shadow-lg shadow-blue-900/20 transition-all active:scale-95 border-2 border-blue-500">
                                Salvar Registro
                            </button>
                        </div>
                    </form>
